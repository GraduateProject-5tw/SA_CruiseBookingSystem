//for individual member, so need to be clean
...
public class MemberController extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        JsonReader jsr = new JsonReader(request);   //老師幫忙寫的，只要把request放進去會回傳json物件
        JSONObject jso = jsr.getObject();
        
        /** 取出被解析到JSON物件之Request參數 */
        String email = jso.getString("email");
        String password = jso.getString("password");
        String name = jso.getString("name");
        
        /** 建立一個新的會員物件 */
        Member m = new Member(email, password, name); //轉成java的物件(member)
        ...
        /** 透過MemberHelper的checkDuplicate()檢查該會員電子郵件信箱是否有重複 */
        else if (!mh.checkDuplicate(m)) {    //檢查商品重複等需要用到資料庫的為JDBC；若與會員有關之農可用memberhelper
