...
public class MemberHelper {
    ...
    public JSONObject getAll() {
        ...
        try {
            ...
            /** 透過 while 迴圈移動pointer，取得每一筆回傳資料 因為回傳到的是結果集合result set*/
            while(rs.next()) {
                /** 每執行一次迴圈表示有一筆資料 */
                row += 1; //第一次就是0-->1行
    ...
    public boolean checkDuplicate(Member m){
        ...
        try {
            ...
            /** 將參數回填至SQL指令當中 */
            pres = conn.prepareStatement(sql); //正在準備statement
            pres.setString(1, email);  //正在準備statement
            /** 執行查詢之SQL指令並記錄其回傳之資料 */
            rs = pres.executeQuery();
    ...
    public JSONObject create(Member m) {
        ...
        try {
            ...
            String status = m.getStatus(); //管理者權限
            ...
            /** 執行新增之SQL指令並記錄影響之行數 */
            row = pres.executeUpdate(); //將資料塞進資料庫
