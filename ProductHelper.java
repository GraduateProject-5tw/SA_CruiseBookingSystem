...
public class ProductHelper {
    ...
    public JSONObject getAll() {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "SELECT * FROM `missa`.`cruises`";
            ...
            while(rs.next()) {
                ...
                /** 將 ResultSet 之資料取出 */
                int cruises_id = rs.getInt("id");
                String name = rs.getString("name");
                String stop_port = rs.getString("stop_port");
                double price = rs.getDouble("price");
                String image = rs.getString("image");
                String describe = rs.getString("describe");
                
                /** 將每一筆商品資料產生一名新Product物件 */
                p = new Product(cruises_id, name , stop_port , price , image, describe);
    ...
    public JSONObject getByIdList(String data) {
      ...
      try {
          ...
          while(rs.next()) {
              ...
              String describe = rs.getString("describe");
              /** 將每一筆商品資料產生一名新Product物件 */
              p = new Product(product_id, name, stop_port, price, image, describe);
              /** 取出該項商品之資料並封裝至 JSONsonArray 內 */
              jsa.put(p.getData());
          }
    ...
    public Product getById(String id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `missa`.`cruises` WHERE `cruises`.`id` = ? LIMIT 1";
